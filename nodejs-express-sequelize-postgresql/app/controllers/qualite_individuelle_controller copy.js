...
const getQualiteIndividuelle = async (req, res) => {
  ...
  for (let i = 0; i < liste_date.length; i++) {
    ...
    for (let j = 0; j < matricules.length; j++) {
      ...
      liste_date[i].liste_matricule[j].taux_nc = Object.keys(donnees).length > 0 ? 100 - donnees[0].taux_nc : '-';
    }
  } 
  ...
  //cadene smiley
  for (let i = 0; i < liste_date.length; i++) {  
    liste_date[i].smiley = compareSmiley(liste_date[i].qualite_atelier, liste_date[i].taux_qualite)
  }
...
const getQualiteDetail = (liste, taux_qualite) => {
  ...
  for (let i = 0; i < liste.length; i++){
    ...
    var qualite_atelier = 100 - (sum_faute / sum_volume_controle)
