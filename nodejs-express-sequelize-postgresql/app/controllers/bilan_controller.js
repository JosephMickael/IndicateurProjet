...
// TODO (!) NE PAS SUPPRIMMER 
const getBilan = async function (req, res, next) {
  ...
  var sql_distinct = "select distinct(id_plan) as plan,libelle_plan from bilan_projet where id_ligne = '" + id_ligne + "' and annee = '" + annee + "' order by libelle_plan";
  ...
  for (let index = 0; index < liste.length; index++) {
    ...
    for (let index2 = 0; index2 < liste_mois_2.length; index2++) {
      ...
      var sql_data = "select mois,coef_p,coef_b,retour_client,felicitation_client,taux_qualite,retard_livraison from bilan_projet where annee = '" + annee + "' and id_ligne = '" + id_ligne + "' and mois = '" + periode + "' and id_plan = '" + id_plan + "' order by libelle_plan";
      ...
      if (liste_data_zero == undefined) {
        liste_data_zero = {
          "mois": periode,
          // "libelle_plan": id_plan,
          "coef_p": null,
          "coef_b": null,
          "retour_client": null,
          "felicitation_client": null,
          "taux_qualite": null,
          "retard_livraison": null
        }
      }
