...
@Component({
  selector: 'app-retard-de-livraison',
  templateUrl: './retard-de-livraison.component.html',
  styleUrls: ['./retard-de-livraison.component.css']
})
export class RetardDeLivraisonComponent implements OnInit {

  constructor(private router: Router, private accueilService: AccueilService,private LpfService: LPFService, private retardService: RetardService, private toast: ToastService,
    ...
    this.LpfService.getListeAnnee().subscribe((data) => {
      ...
      for (let item in this.annees) {
        ...
        if (year == annee) {
          ...
          this.afficherRetard(this.id_ligne, this.annee)
        }
      }
    });
  }
  ...
  afficherRetard(id_ligne, annee) {
    this.isLoading = true; 
    this.retardService.getRetard(id_ligne, annee).subscribe((data) => {
      var donneeS = JSON.stringify(data)
      var donneeP = JSON.parse(donneeS)
      this.liste_mois = donneeP.mois; 
      this.liste_retard = [];
      this.liste_retard.push(donneeP.newliste)
  ...
  exportToExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'retard de livraison');
    ...
    const fileName = `retard_livraison_${this.id_ligne}_${this.annee}.xlsx`;
